...
export default function ExpenseAIncome({listing}) {
  const income = (listing ?? []).filter((item) => item.noteType === "Thu tiền").reduce((acc, item) => acc + item.noteMoney, 0);
  const expense = (listing ?? []).filter((item) => item.noteType === "Chi tiền").reduce((acc, item) => acc + item.noteMoney, 0);  
